...
import ShipmentReviewPanel from '@/components/reviews/ShipmentReviewPanel';
...
export default function ShipmentOffersPage() {
    ...
    const handleCounterOffer = async (offerId: string, price: number, message?: string) => {
                ...
                <div className="grid grid-cols-1 xl:grid-cols-12 gap-8">
                        ...
                        <section className="glass-panel p-6 rounded-3xl">
                            ...
                            <div className="space-y-8">
                                ...
                                <div className="flex flex-col md:flex-row gap-4 items-stretch">
                                    ...
                                    {/* Map */}
                                    <div className="w-full md:w-64 lg:w-80 h-48 md:h-56 lg:h-64 rounded-2xl overflow-hidden shadow-sm border border-gray-100 relative bg-gray-50 group">
                                        {shipment?.distance && (
                    ...
                    <div className="xl:col-span-7">
                        <div className="sticky top-8 space-y-6">
                            {shipment?.status === 'DELIVERED' && shipment?.id && (
                                <PaymentPanel shipmentId={shipment.id} justReturnedStatus={paymentReturn} />
                            )}

                            {shipment?.status === 'DELIVERED' && shipment?.id && (
                                <ShipmentReviewPanel shipmentId={shipment.id} />
                            )}
                                            ...
                                            >
                                                <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                                                    ...
                                                    <div className="flex-1 min-w-0">
                                                        <div className="flex items-center gap-3 mb-1">
                                                            {offer.status === 'COUNTERED' ? (
                                                                <>
                                                                    <h3 className="text-lg font-bold text-gray-400 line-through tracking-tight">
                                                                        €{offer.offered_price.toLocaleString()}
                                                                    </h3>
                                                                    <h3 className="text-3xl font-bold text-blue-700 tracking-tight">
                                                                        €{(offer.counter_price ?? offer.offered_price).toLocaleString()}
                                                                    </h3>
                                                                </>
                                                            ) : (
                                                            ...
                                                            {offer.status === 'COUNTERED' && (
                                                                <span className="px-2.5 py-0.5 bg-amber-50 text-amber-700 text-[10px] font-bold uppercase tracking-wider rounded-md border border-amber-100">
                                                                    {offer.countered_by === 'SHIPPER' ? 'Your Counter' : 'Carrier Countered'}
                                                                </span>
                                                            )}
                                                        ...
                                                        <div className="mt-1.5">
                                                            <CarrierRatingBadge carrierId={offer.carrier_id} compact />
                                                        </div>
                                                        ...
                                                        {offer.status === 'COUNTERED' && offer.counter_message && (
                                                            <div className="mt-2 bg-amber-50/60 rounded-lg p-3 text-sm text-amber-800 italic border border-amber-100/50 max-w-xl">
                                                        ...
                                                        {counteringOfferId === offer.id && (
                                                            <CounterOfferPanel
                                                                ...
                                                                onSubmit={(price, message) => handleCounterOffer(offer.id, price, message)}
                                                            />
                                                        )}
                                                    </div>
                                                    ...
                                                    <div className="flex items-center gap-2 self-start md:self-center pt-2 md:pt-0">
                                                        {(offer.status === 'PENDING' || (offer.status === 'COUNTERED' && offer.countered_by === 'CARRIER')) ? (
                                                            <>
                                                                <button
                                                                ...
                                                                <button
                                                                    ...
                                                                    className="px-4 py-2 rounded-xl text-blue-600 font-bold text-xs hover:bg-blue-50 transition-all"
                                                                >
                                                                    Counter
                                                                </button>
                                                                <button
                                                            ...
                                                            <span className="text-amber-500 text-xs font-bold italic px-4">
                                                                Waiting for carrier
                                                            </span>
                                                        ) : (
                                                            <span className="text-gray-300 text-xs font-bold italic px-4">
                                                                {offer.status === 'ACCEPTED' ? 'Offer Accepted' : offer.status === 'WITHDRAWN' ? 'Withdrawn by Carrier' : offer.status === 'EXPIRED' ? 'Offer Expired' : 'Offer Rejected'}
                                                            </span>
                                                        )}
                                                    </div>
                                                </div>
